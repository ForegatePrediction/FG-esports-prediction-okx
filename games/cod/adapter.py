#!/usr/bin/env python3
"""使命召唤(Call of Duty / CDL)适配器:PandaScore 系列数据 → 按地图展开(队伍级)。"""
...
def load(data_dir=None):
    ...
    cand = glob.glob(os.path.join(data_dir, "cod*_matches.json"))
    ...
    for x in raw:
        if not (isinstance(x["s1"], int) and isinstance(x["s2"], int) and x["w"] in (1, 2)):
            continue
        if x["s1"] == x["s2"] or x["bo"] not in (1, 3, 5, 7, 9) or not x["date"] or x["date"] > today:
            continue
