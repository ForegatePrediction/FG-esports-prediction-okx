#!/usr/bin/env python3
"""决胜巅峰 / Mobile Legends(MLBB)适配器:PandaScore 系列数据 → 按小局展开(队伍级)。"""
...
def load(data_dir=None):
    ...
    raw = json.load(open(os.path.join(data_dir, "mlbb_matches.json")))
    today = datetime.date.today().isoformat()
    out = []
    for x in raw:
        if not (isinstance(x["s1"], int) and isinstance(x["s2"], int) and x["w"] in (1, 2)):
            continue
        if x["s1"] == x["s2"] or x["bo"] not in (1, 2, 3, 5, 7) or not x["date"] or x["date"] > today:
            continue
